...
- a grounded model viewer with a dominant-face-first generic pose solver, support-aware placement, final settling and conservative surface-leveling polish, deterministic first-load framing, compact review telemetry, a consolidated bottom footer/meta status band, a lower section that behaves as three user-facing groups for model info, printability, and mesh readiness, tighter lower-band and footer fit/stability polish for longer values and live generation states, normalized printability row rhythm, in-view overlay navigation controls, a lower-right real 3D orientation cube with calmer teal-led shading that now reflects the same effective basis as the main viewer, a persistent viewer status card, a calm icon-based auto-orbit toggle, a same-shell top-level mode system with real Workspace and Models views, a cleaned-up content-first saved-model browser in Models with a synced sidebar list, and a right-side read-only properties panel with a separately framed actions area for real next-step controls
